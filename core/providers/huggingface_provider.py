"""HuggingFace Inference API translation provider adapter."""
...
class HuggingFaceProvider(BaseTranslationProvider):
    ...
    def translate(self, request: TranslationRequest, candidate: ProviderCandidate | None = None) -> TranslationResult:
        ...
        api_token = self.api_key_pool[candidate.key_index] if (self.api_key_pool and candidate.key_index >= 0) else ""

        # Endpoint format: https://api-inference.huggingface.co/models/{model_name}
        endpoint = f"{self.base_url.rstrip('/')}/{model_name}"
        ...
        try:
            ...
            # HuggingFace returns a list of items: [{"generated_text": "..."}]
            # Or a dict with error if failed
